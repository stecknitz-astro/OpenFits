...
type
  ...
  public
    constructor CreateJoin(AStream: TStream);
    constructor CreateMade(AStream: TStream; const AHduCore: THduCore2d);
  public
    // Property override HduCore to HduCore2d
    property HduCore: THduCore2d read GetHduCore2d;
    ...
    // Full data region
    property DataRgn: TRgn read GetDataRgn;
...
begin
  with AHduCore do
  begin
    E := Simple and (BitPix > biUnknown) and (NAxis <= cMaxNaxis) and (NAxis = Length(NAxisn));
    I := 0;
    while E and (I < NAxis) do
    begin
      E := NAxisn[I] > 0;
      Inc(I);
    end;
...
begin
  ...
  try   
    ...
    for I := 0 to cCountLinesInBlock - 1 do
    begin
...
begin
  ...
  FStream := nil;
  FHduKeywords := nil; 
  FHduCore := ToHduCore(False, biUnknown, [], 1.0, 0.0); 
end;
...
procedure TFits.MakeFromStream;
var
  I, J, N, Count: Integer;
  K: TKindHduKeyword;
  S: string;
...
begin
  ...
  try
    ...
    Count := 0;
    while (not Find[keyEnd]) and ((Int64(Count) * cSizeLine + cSizeBlock) <= FStream.Size) do
    begin
      StreamRead(Int64(Count) * cSizeLine, cSizeBlock, Buffer);
      for I := cCountLinesInBlock - 1 downto 0 do
        if CompareMem(@Buf[0], @Buffer[I * cSizeLine], cSizeKeyword) then
        begin
          Find[keyEnd] := True;
          Break;
        end;
    ...
    SetLength(FHduKeywords, Count);    
    S := '';
    for I := 0 to (Count div cCountLinesInBlock) - 1 do
    begin
      StreamRead(Int64(I) * cSizeBlock, cSizeBlock, Buffer);
      for J := 0 to cCountLinesInBlock - 1 do
      begin
    ...
    begin
      ...
      if K = keyEnd then
        Break;
      if K in [keySimple, keyBitPix, keyNAxis, keyBScale, keyBZero] then
      if not Find[K] then
      begin
        Find[K] := True;
        StreamRead(Int64(I) * cSizeLine, cSizeLine, Buffer);
        CopyTo(Buffer, 0, cSizeLine, S);
        S := LineToItem(S).Value.Str;
        case K of
          keySimple:
            FHduCore.Simple := StriToBol(S);
          keyBitPix:
            FHduCore.BitPix := StriToBitPix(S);
          keyNAxis:
            begin
          ...
          keyBScale:
            FHduCore.BScale := StriToFloat(S);
          keyBZero:
            FHduCore.BZero := StriToFloat(S);
        end;
      end;
    end;
    // Definition of NAxisn in core
    if Find[keyNAxis] then
    begin
      I := 0;
      N := 0; // number of axes found
      while (I < Count) and (N < FHduCore.NAxis) do
      begin
        K := KeywordKind(FHduKeywords[I]);
        case K of
          keyEnd:
          ...
          keyNAxisn:
            begin
              ...
              if (J >= 0) and (J < FHduCore.NAxis) then
              begin
                StreamRead(Int64(I) * cSizeLine, cSizeLine, Buffer);
                CopyTo(Buffer, 0, cSizeLine, S);
                S := LineToItem(S).Value.Str;
  ...
  case Cast of
    ...
    cCastCharsLong:
      begin
        ...
        while Length(Line) >= cSizeLine do
        begin
...
begin
  ...
  begin
    ...
    for I := StartIndex to FLineCount - 1 do
      if FHduKeywords[I] = Keyword then
      begin
        Result := I;
        Break;
      end;
...
begin
  ...
  try
    ...
    if Grow > Stock then // Stock lines not enough, FLineCapacity & FDataOffset must change
    begin
    ...
    else {if Grow <= Stock then} // Stock lines are enough, FLineCapacity & FDataOffset remain unchanged
    begin
      // Move stream header-content
      AOffset := Int64(Index) * cSizeLine;
      ACount := (Int64(FLineCount) - Index) * cSizeLine;
...
begin
  inherited;
  //!!if AHduCore.NAxis <> 2 then
  if AHduCore.NAxis > 3 then
    raise EFitsHduCoreException.Create(eCoreTwoInvalid, ERROR_CORE_TWO_INVALID);
end;
...
begin
  ...
  if not RgnIsIncluded(DataRgn, Rgn) then
    with DataRgn do
      raise EFitsFrameException.CreateFmt(eDataRgnInvalid,
                                          [Rgn.X1, Rgn.Y1, Rgn.ColsCount, Rgn.RowsCount, X1, Y1, ColsCount, RowsCount],
                                          ERRROR_DATAACCESS_RGN_INVALID);
  if (Data = nil) then
...
begin
  ...
  if not (Assigned(FProcRead) and Assigned(FProcWrite)) then
    raise EFitsFrameException.CreateFmt(eBitPixSupport, [BitPixToInt(DataBitPix)], ERRROR_DATAACCESS_PROC_NIL);
  if not RgnIsIncluded(DataRgn, X, Y) then
    with DataRgn do
      raise EFitsFrameException.CreateFmt(eDataPixelInvalid, [X, Y, X1, Y1, ColsCount, RowsCount], ERRROR_DATAACCESS_PIXEL_INVALID);
end;
...
constructor TFitsFrame.CreateMade(AStream: TStream; const AHduCore: THduCore2d);
var
  ACore: THduCore;
begin
  ACore := HduCore2dToMult(AHduCore);
  try
...
begin
  DataPrepare(Data, DataRgn);
end;

procedure TFitsFrame.DataRead(Data: Pointer);
begin
  DataRead(Data, DataRgn);
end;
...
begin
  ...
  //!!
  Result.NAxis3 := FHduCore.NAxisn[2];
...
function TFitsFrame.GetDataRgn: TRgn;
begin
  Result := ToRgnCoRo(0, 0, FHduCore.NAxisn[0], FHduCore.NAxisn[1]);
end;

function TFitsFrame.GetHduCore2d: THduCore2d;
begin
  Result := HduCoreMultTo2d(FHduCore);
end;
...
constructor TFitsFileFrame.CreateMade(const AFileName: string; const AHduCore: THduCore2d);
var
  F: TFileStream;
begin
  if not FileExists(AFileName) then
